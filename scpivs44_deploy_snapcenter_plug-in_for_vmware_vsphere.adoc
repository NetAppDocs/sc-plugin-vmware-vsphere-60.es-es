...
.Pasos
. Para VMware vCenter 7.0.3 y versiones posteriores, siga los pasos de para importar los certificados en link:scpivs44_download_the_ova_open_virtual_appliance.html["Descargue el dispositivo Open Virtual Appliance (OVA)"^] vCenter.
...
. Extraiga el archivo .tar, que contiene el archivo .ova en su sistema local. En la página *Seleccione una plantilla OVF*, especifique la ubicación del `.ova` archivo dentro de la carpeta .tar extraída.
...
+
Este paso especifica dónde importar `.tar` el archivo en vCenter. El nombre predeterminado de la máquina virtual es el mismo que el nombre del archivo seleccionado `.ova` . Si cambia el nombre predeterminado, elija un nombre único dentro de cada carpeta de máquina virtual de vCenter Server.
...
. En la página *Detalles de la revisión*, verifique `.tar` los detalles de la plantilla y haga clic en *Siguiente*.
...
+
En el campo *vCenter username*, introduzca el nombre de usuario con formato `domain\username`.
...
+
Utilice el formato `\https://<appliance-IP-address>:8080` para acceder a la interfaz gráfica de usuario de gestión.
...
.Después de terminar
Debe completar los requisitos link:scpivs44_post_deployment_required_operations_and_issues.html["operaciones posteriores a la implementación"].
